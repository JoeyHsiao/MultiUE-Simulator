...
set "default_http_dl_cmd=cd /data; TESTING_TIMES; do killall curl; cur_time=$(date '+%%Y%%m%%d_%%H%%M%%S'); filename=http_${cur_time}_NAMEDEV.log; echo $filename >> /xq3_logs_file; curl -o /dev/null http://SERVER_IP/DL_FILE_NAME 2>&1 | tee /data/$filename; if [ "$(cat /var/xq3/net_ifstatus_state)" = "0" ]; then echo "network_disconnect" >> /xq3_logs_file; DO_BREAK fi ;done"
